...
if(isset($_POST['submit']))
{
	...
	$pre_count="select id from membership_plan where user_id='$user_id' and status='1'";
	$pastcount=mysqli_num_rows(mysqli_query($conn,$pre_count));
	if($pastcount==0)
	{
	$assign_user="y";
	$default_plan="y";
	...
	mysqli_query($conn, "INSERT INTO `membership_plan` (`user_id`,`token_id`,`plan_name`, `plan_desc`, `plan_img`, `plan_amount`, `plan_benefit`, `total_min_order_amount`, `total_max_order_amount`, `valid_from`, `valid_to`, `plan_type`, `validity`, `created`, `status`,`assign_user`,`default_plan`) 
	VALUES ('$user_id','$token_id','$name', '$plan_desc', '$file_name', '$amount', '$benefit', '$total_min_order_amount', '$total_max_order_amount', '$valid_from', '$valid_to', '$type', '$validity', '$created', '$status','$assign_user','$default_plan')");
