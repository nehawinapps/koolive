...
/*	$already_exists = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users WHERE email='$email'"));
	...
	$already_exists1 = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users WHERE mobile_number='$cm'"));
...
*/
	if($error == "")
	{
	 ...
	 mysqli_query($conn, "INSERT INTO users SET name='$name',user_roles='$user_role', password='$password', joined='".time()."', isLocked='1', verification_code='$code', referral_id='$ref',referred_by='$reffered_by',security_answer= '$security',security_questions= '$questions',fund_password='$fund_pass',email='$email',mobile_number='$cm'");
		 
		$user_id = mysqli_insert_id($conn);
			$current_url = "http://".$_SERVER['HTTP_HOST']."".$_SERVER['REQUEST_URI'];

	Print("Sending to one way sms " . gw_send_sms("APIHKXVL33N5E", "APIHKXVL33N5EHKXVL", "9787136232", "$cm", "Verify Your Account on KooExchange $current_url?code=$code&id=$user_id"));
		$current_url = "http://".$_SERVER['HTTP_HOST']."".$_SERVER['REQUEST_URI'];
		$subject = "Verify Your Account | KooExchange";

		$message = "
		<html>
		<head>
		<title>Verify Your Account | KooExchange</title>
		</head>
		<body>
		<h3>Verify Your Account on KooExchange</h3>
...
if(isset($_POST['login']))
{
	...
	if($error == "")
	{
		$user_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id,isLocked FROM users WHERE mobile_number='$cm' AND password='$password'"));
		$id = $user_row['id'];
...
if(isset($_POST['forget']))
{
	...
	if($error == "")
	{
		$subject = "Forget Password | KooExchange";

		$message = "
		<html>
		<head>
		<title>Forget Password | KooExchange</title>
		</head>
		<body>
		<p>Password for your Account ($email) : $password</p>
		</body>
		</html>
		";
                ...
                <div class="sap_tabs">
                    <div id="horizontalTab" style="display: block; width: 100%; margin: 0px;">
                        <ul class="resp-tabs-list">
							...
							<li class="resp-tab-item" aria-controls="tab_item-2" role="tab"><label>/</label><span>Forget Password</span></li>
                            <div class="clear"> </div>
                        </ul>
	...
	<option data-countryCode="GB" value="44">UK (+44)</option> 
		...
		<option data-countryCode="ZW" value="263">Zimbabwe (+263)</option>
                                    ...
                                    <div class="login-bottom login-bottom1">
                                        ...
                                        <div class="clear" style="padding:27px;"></div>
                                    </div>
                                </div>
                                </form>
                            </div>
                            <div class="tab-1 resp-tab-content" aria-labelledby="tab_item-1">
								...
								<div class="login-top sign-top">
									...
									<input type="text" class="name active" placeholder="User Name" name="name" value="<?php isset($name) ? $name : ""; ?>" id="reg_name" />
									<select name="countrycode" id="countrycode" class="ct_ctycode">
	<option data-countryCode="MY" value="60">Malaysia (+60)</option>
	<option data-countryCode="CN" value="86">China (+86)</option>
	<option data-countryCode="TH" value="66">Thailand (+66)</option>
		...
		<option data-countryCode="ZW" value="263">Zimbabwe (+263)</option>
							...
							<div class="tab-1 resp-tab-content" aria-labelledby="tab_item-2">
								...
								<div class="login-top sign-top">
                                    ...
                                    <div class="login-bottom">
                                        <div class="submit">
                                                <input type="submit" value="SUBMIT" name="forget" />
                                        </div>
