...
class SectionTable
{
	...
	public function create($data)
	{
		...
		$qr_link=$data['qr_link'];
		 $query = "INSERT INTO $tableName (section_id, name, description,qr_link) VALUES('$section_id','$name','$description','$qr_link')";
		$stmt = mysqli_query($this->connection, $query);
		return true;
		...
		// if ($stmt = mysqli_prepare($this->connection, $query)) {
			...
			// $qr_link = $data['qr_link'];
			// $status = (bool)$status;
			// mysqli_stmt_bind_param($stmt, "issi", $sectionId, $name, $description, $status,$qr_link);
			// echo $stmt;
			// die;
