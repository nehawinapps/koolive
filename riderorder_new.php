...
//    $query5="INSERT INTO rider assign_time values('$assign_time')
...
if(isset($_POST['a'])) //edit hidden field
{
 //echo "<br>edit";
	$a=$_POST['a'];
	//echo $assign_time;
 ...
 <?php
    ...
    // $query2="INSERT INTO rider(rider_code) values($r_code)";
    // mysqli_query($conn,$query2);
    ...
    //     $query2="INSERT INTO riders(product_id) values($r)";
    //      mysqli_query($conn,$query2);
...
{
	$sid=$_GET['sid'];
	$url="riderorder_new.php?ms=".md5(rand());
...
users as u on u.id=order_list.merchant_id  left join rider on order_list.rider_id=rider.Id  order by order_list.id desc limit 0,30 ";

    // $query="SELECT * FROM users 
	// INNER JOIN order_list ON users.id=order_list.user_id where rider_code='$r_code' ";
...
<script>
	function myFunction() {
        ...
        <button class="btn btn-info assign_order" id="myDIV" onclick="myFunction()"  type="button" data-target="#orderdetailmodel<?php echo $r['id'];?>" data-toggle="modal"   value="<?php echo $r_n; ?>">assign </button>
		<input type="hidden" name="a" value="<?php echo $r['id']; ?>"> 
		<!-- edit click value pass -->
...
?>
		...
		<div class="modal fade" id="orderdetailmodel<?php echo $r['id'];?>" role="dialog" style="margin-top:12%;">
...
<div class="modal fade" id="orderdetailmode" role="dialog" style="margin-top:12%;"> 
							<div class="modal-dialog">
							<!-- Modal content-->		
							<div class="modal-content">	
							<div class="modal-header">	
 ...
 if($_POST['access'] && $_POST['r_code'] && $_POST['access_order_id'])
	{
		...
		{
			$q="select * from rider where rider_code='$r_code'";
			...
			if($riderdata)
			{
				...
				header("Location:https://localhost/koolive/riderorder_new.php");
			}	
		}
		else
		{
		 ...
		 $("#orderdetailmodel").modal("show"); 
	  	    ...
	  	    setInterval(function(){ 
					...
					var s_token=generatetokenno(16);
						var r_url="https://localhost/koolive/riderorder_new.php?ms="+s_token;
...
$(document).ready(function(){
    $('#orderdetailmodel').on('show.bs.modal', function (e) {
        var rowid = $(e.relatedTarget).data('id');
        $.ajax({
            type : 'post',
            url : 'riderorder_new.php', //Here you will fetch records 
