...
<?php  $login_user_id=$_SESSION['login'];
 ...
 ?>
<!DOCTYPE html>
<html lang="en" style="" class="js flexbox flexboxlegacy canvas canvastext webgl no-touch geolocation postmessage websqldatabase indexeddb hashchange history draganddrop websockets rgba hsla multiplebgs backgroundsize borderimagec borderradius boxshadow textshadow opacity cssanimations csscolumns cssgradients cssreflections csstransforms csstransforms3d csstransitions fontface generatedcontent video audio localstorage sessionstorage webworkers applicationcache svg inlinesvg smil svgclippaths">
...
<body  class="header-light sidebar-dark sidebar-expand pace-done">
    ...
    <?php } ?>
        ...
        <form method="post" id="order_place" action="order_cash3.php">
        <?php
        $stl_key = rand();
        ...
        <?php } else { ?>
        <form id="order_place" action="order_cash3.php" method="post">
        <?php } ?>
   ...
   <?php echo $language['we_can_contact']; ?>
    ...
    <?php } ?>
	 ...
	 <div class="" style="display:none;" id="delivery_label">  
		...
		<div style="display:none;" class="coupon_discount_label">  
			<div style="grid-template-columns:.2fr 2fr;grid-column-gap: 10px;vertical-align: middle;align-content:center;font-weight: bold;font-size: 15px;">

			   
				<span class="coupon_discount_value" id="message1"></span>
			</div>
    ...
    <div class="row">
		...
		<input type="hidden" value='0' id="coupon_discount_amount" name="coupon_discount_amount"/>
      ...
      <div class="modal-content">
        ...
        <div class="modal-header">
		  ...
		  <div class="form-group" style="margin-bottom: 0em;">
			...
			<input type="hidden" id="input_final_amount">
...
$(document).ready(function(){
	 ...
	 if(today_limit=="expire")
	 {
		...
		var msg="In a day you can only place 10 order,Try to order With different merchant";  
		$('#show_msg').html(msg);
...
if(p_status=="paypalcancel")
{
  ...
  // $('#confm').click(function(){
       // $('#order_place').attr('action', 'order_cash3.php');
	...
	// code for rebeat process by mayank 
	 ...
	 $(".wallet_select").click(function(){
	  ...
	  $(".wallet_final_payment").click(function(){
		...
		var coupon_discount=$('#coupon_discount_amount').val();
		if(parseFloat(coupon_discount)>0)
		{
...
function totalcart()
{
	...
	if(membership_discount_input)
	{
		...
		var coupon_min = $('#coupon_min_value').val();
		var coupon_max = $('#coupon_max_value').val();
		if(totalsale >= coupon_min && totalsale <= coupon_max){
		    if($('#coupon_charges').val() == 0){
    		    $('#coupon_charges').val(0);
    		    var coupon_discount = 0;
    		    var $messageDiv = $('#message');
                var $messageDiv1 = $('#message1');
                $messageDiv.hide();
                $messageDiv1.hide();
    		}else{
    		    var coupon_type = $('#coupon_type').val();
    		    var coupon_discount = $('#coupon_charges').val();
    		    var $messageDiv1 = $('#message1');
    		    if(coupon_type == 'per'){
    		        coupon_discount = totalsale * coupon_discount;
    		       $messageDiv1.show().html('Coupon Discount : Rm '+$discount1.toFixed(2));
    		    }else{
					$messageDiv1.show().html('Coupon Discount : Rm '+$discount1.toFixed(2));
    		    }
            ...
            if ($('#coupon_code').is(":empty")) {
                ...
                var $messageDiv = $('#message');
                $messageDiv.show().html('Price not in range');
                var $messageDiv1 = $('#message1');
                $messageDiv1.hide();
            }
		}
		...
		if(membership_discount)
		{
			...
			$('.final_amount_value').html(final_charge.toFixed(2)); 
			$('#input_final_amount').val(final_charge.toFixed(2));
		}
   ...
   // var user_mobile=$('#mobile_number').val();
		...
		if(s_flag)
		{
			...
			$.ajax({
				...
				success: function(response) {
					var result = JSON.parse(JSON.stringify(response));
					if(result.status){
						$messageDiv.show().html(result.data);
						if(result.type == 'per'){
							$discount1 = parseFloat(total_amount) * parseFloat(result.price);
							var final_amount = $('#input_final_amount').val();
							final_amount = parseFloat(final_amount) - parseFloat(sum * result.price);
							$('.final_amount_value').html(final_amount.toFixed(2));
							$('.coupon_discount_amount_label').show();
							$('.coupon_discount_amount_value').html(sum * result.price);
							$('.coupon_discount_label').show();
							$('#coupon_charges').val(result.price);
							$messageDiv1.show().html('Coupon Discount : Rm '+$discount1.toFixed(2));
							
						}else{
							$('#coupon_charges').val(result.price);
							$('.coupon_discount_label').show();
							$messageDiv1.show().html('Coupon Discount : Rm '+result.price);
							$discount1 = sum - result.price;
							var final_amount = $('#input_final_amount').val();
							final_amount = parseFloat(final_amount) - parseFloat(result.price);
							$('.final_amount_value').html(final_amount.toFixed(2));
							$('.coupon_discount_amount_label').show();
						...
						if ($('#confm').hasClass("disabled") == 1) {
							$('#confm').removeClass("disabled");
						}
						if ($('.online_pay').hasClass("disabled") == 1) {
							$('.online_pay').removeClass("disabled");
						}
						$('#coupon_discount_amount').html(parseFloat(result.price));
						$('#coupon_discount_amount').val(parseFloat(result.price));
						$('#coupon_discount_amount_label').html(parseFloat(result.price));
					}else{
						$messageDiv.show().html(result.data);
						$messageDiv1.hide();
						if ($('#confm').hasClass("disabled") == 0) {
							$('#confm').addClass("disabled");
						}
						if ($('.online_pay').hasClass("disabled") == 0) {
							$('.online_pay').addClass("disabled");
						}
					}
				}
			});
    ...
    $('#coupon_code').blur(function () {
      if ($('input:text').is(":empty")) {
          ...
          var final_value = $('#input_final_amount').val();
        ...
        $messageDiv.hide();
        $('#coupon_charges').val(0);
        $('.final_amount_value').html(final_value);
      }
    });
    $('#coupon_code').keypress(function () {
        if($('input:text').length > 0 && $('input1').val() != ''){
            if ($('#confm').hasClass("disabled") == 0) {
           		$('#confm').addClass("disabled");
        	}
        	if ($('.online_pay').hasClass("disabled") == 0) {
