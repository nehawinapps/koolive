...
// }
  ...
  // die;
   ...
   if($_SESSION['login'])
   {
     ...
     $query="SELECT order_voice_list.*, sections.name as section_name FROM order_voice_list left join sections on order_voice_list.section_type = sections.id WHERE order_voice_list.user_id ='".$user_id."' ORDER BY `created_on` DESC LIMIT $limit";

     $total_rows = mysqli_query($conn,$query);
    
   $user_order = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM order_voice_list WHERE user_id ='".$user_id."' ORDER BY `created_on` DESC limit 0,1"));
          ...
          <div class="well">
            ...
            <div class="row">
                ...
                <?php
                    ...
                    if($today == $today_order[0] && $status1 == 0 ){ ?>
            ...
            <tbody>
              ...
              <td style="text-align: center;">
                <audio id="player_<?php echo $i; ?>" style="display: none;" src="<?php echo $row['file_path']; ?>">
                </audio>
                <button type="button" id="buttonPlay" data-id="<?php echo $i; ?>" class="btn btn-info soundIcnPlay row_class">
             ...
             <td><?php $show_date=$row['created_on']; echo $newDate = date("d-m-Y h:i A", strtotime($show_date));   ?></td>
              ...
              <td>
			   <a href="<?php echo $site_url; ?>/view_merchant.php?sid=<?php echo $user_name['mobile_number'];?>"><?php echo $user_name['name'];  ?></td>
            
			  </td>
              <td><a href=''><?php echo $section_type['name'];?></a></td>
              <td><?php echo $row['table_type'];?></td>
              <td><?php echo $row['user_mobile'];?></td>
              <td class="location_<?php echo $row['id']; ?>">
